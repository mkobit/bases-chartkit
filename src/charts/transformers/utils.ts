...
export function safeToString(val: unknown): string {
  ...
  if (isRecord(val) && isRenderableValue(val)) {
    // eslint-disable-next-line @typescript-eslint/no-base-to-string -- isRenderableValue narrows toString to `() => string`, but the rule's type resolution doesn't see through the Record<string, unknown> intersection.
